...
class DataSourceManager(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.auto_generate_btn = QPushButton("⚡ Auto Generate Workflow")
        self.auto_generate_btn.setStyleSheet("""
            ...
            QPushButton:disabled { background: #ccc; }
        """)
        self.auto_generate_btn.clicked.connect(self._auto_generate_workflow)
        self.auto_generate_btn.setEnabled(False)
        preview_toolbar.addWidget(self.auto_generate_btn)
    ...
    def _auto_generate_workflow(self):
        """Auto-generate workflow dari data Excel yang di-preview."""
        if not self._preview_data_rows:
            QMessageBox.warning(self, "No Data",
                "Preview data terlebih dahulu sebelum auto-generate workflow.")
            return

        try:
            # Ambil headers dari preview
            headers = list(self._preview_data_rows[0].keys())
            if not headers:
                QMessageBox.warning(self, "Invalid Data", "Data preview tidak memiliki kolom.")
                return

            # Import dialog konfigurasi
            from frontend.ui.auto_generate_dialog import AutoGenerateDialog
            dialog = AutoGenerateDialog(headers, self)

            if dialog.exec():
                config = dialog.get_config()
                self._build_workflow_from_excel(headers, config)

        except Exception as e:
            QMessageBox.critical(self, "Error", f"Gagal generate workflow:\n{str(e)}")

    def _build_workflow_from_excel(self, headers, config):
        """Buat workflow dari data Excel berdasarkan konfigurasi dialog."""
        try:
            import json
            ...
            # Generate steps berdasarkan konfigurasi
            steps = []
            step_counter = [0]
            ...
            for header in headers:
                field_config = config.get("field_mappings", {}).get(header, {})
                action_type = field_config.get("action_type", "input_text")
                params = field_config.get("params", {})

                # Default params berdasarkan action_type
                if action_type == "input_text":
                    params.setdefault("selector", f"input[name='{header}']")
                    params.setdefault("value", f"{{{{data.{header}}}}}")
                elif action_type == "click":
                    params.setdefault("selector", f"button[name='{header}']")
                elif action_type == "select":
                    params.setdefault("selector", f"select[name='{header}']")
                    params.setdefault("select_value", f"{{{{data.{header}}}}}")
                elif action_type == "upload_file":
                    params.setdefault("selector", f"input[type='file'][name='{header}']")
                    params.setdefault("file_path", f"{{{{data.{header}}}}}")
                ...
                loop_step["steps"].append(step)

            steps.append(loop_step)

            # 4. Submit step (jika ada)
            if config.get("submit_selector"):
                steps.append({
                    ...
                    "label": "Submit Form",
                    "params": {
                        "selector": config.get("submit_selector"),
                    },
                    "on_error": "stop",
                    "retry": {"max_retries": 3, "delay": 2000},
                })
            ...
            # Parse dan load ke editor
            workflow = parser.parse(workflow_data)
            main_window = self._get_main_window()
            if main_window:
                main_window.current_workflow = workflow
                main_window.workflow_editor.load_workflow(workflow)
                main_window.execution_panel.set_workflow(workflow)
                main_window._mark_modified()
                QMessageBox.information(self, "Success",
                    f"Workflow berhasil di-generate!\n\n"
                    f"Total steps: {len(steps)}\n"
                    f"Data rows: {len(self._preview_data_rows)}\n\n"
                    f"Workflow siap dijalankan.")
            else:
                # Fallback: emit signal dengan workflow data
            ...
            QMessageBox.critical(self, "Error", f"Gagal build workflow:\n{str(e)}")

    def _get_main_window(self):
        """Dapatkan instance MainWindow dari parent hierarchy."""
        widget = self
        while widget:
            if hasattr(widget, '_on_workflow_changed'):
                return widget
